...
### Added (Studio and Pages Ecosystem)
- Added a visible **OS Downloads** link in the Studio sidebar that opens the OkamaOS Pages downloads section.
- Added Dashboard actions for **Download OkamaOS** and **OS Manual** so creators can move from Studio to ISO releases, update docs, and package docs without guessing the next URL.
- Added Studio-local `VERSION`, `CHANGELOG.md`, and `ROADMAP.md` files for Studio release tracking.
- Replaced the scaffolded Studio README with ecosystem workflow documentation and direct Pages links.

### Changed
- Bumped repo tracker version to `1.3.1` and Studio package metadata to `0.3.1`.
- Aligned the Buildroot runtime package metadata version constant with the existing package `VERSION` file.
- No new OS runtime update bundle is generated by this Studio/Pages link change.
